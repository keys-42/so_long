...
/*   Updated: 2022/12/10 00:16:11 by keys             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
		printf("(%d)", data->maps->collection_num);
	data->mlx = mlx_init();
	if (!data->mlx)
		ft_free_data(&data, 1, "Could not initialize mlx.");
	data->mlx_win = mlx_new_window(data->mlx, WINDOWSIZE * (data->width),
			WINDOWSIZE * (data->hight), "so_long");
	mlx_image_init(data);
